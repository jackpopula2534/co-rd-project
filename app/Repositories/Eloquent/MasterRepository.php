...
//ประกาศว่ารอบรับ Interface โดยเงือนไข Repository จะต้องมีเมธอดทั้งหมดที่ Interface กำหนดไว้
{
    ...
    private function buildSearchQuery($query, $param, $searchFields, $withRelations)
    {
        if (isset($param['searchValue'])) {

            foreach ($withRelations as $relation) {
                $query->orWhereHas($relation, function ($query) use ($param, $searchFields, $relation) {
                    foreach ($searchFields as $field) {
                        if (is_array($field)) { // Check if $field is an object
                            $fieldName = array_key_first($field); // Get the key (field name)
                            $relationField = $field[$fieldName]; // Get the value (related field name)

                            if ($fieldName === $relation) { // Check if field name matches relation
                                $query->where($relationField, 'like', '%' . $param['searchValue'] . '%');
                            }
                        }
                    }
                });
    ...
    public function paginate($param, ?array $searchFields = null, ?array $withRelations = null, ?array $where = null, $whereRaw): Collection
    {
        ...
        $query = $this->buildSearchQuery($query, $param, $searchFields, $withRelations);
