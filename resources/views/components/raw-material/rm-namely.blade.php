                <li class="pt-3 m-2">
                    <div class="relative">

                        <div class="pt-1">
                            <div
                                class="mt-2 rounded-lg border border-gray-brown-500  w-full px-4 py-2 border-b border-gray-200 rounded-t-xl dark:border-gray-600 bg-gray-brown-200 text-gray-brown-500 font-medium text-xs">
                                <p class="m-2">
                                    {{ $rmDetail['rm_namely'] ?? '-' }}
                                </p>
                            </div>
                        </div>


                        <div class="fixed uppercase absolute top-0 left-0 pl-4">
                            <span
                                class="bg-gray-brown-500 text-white text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">namely</span>
                        </div>

                    </div>
                </li>
